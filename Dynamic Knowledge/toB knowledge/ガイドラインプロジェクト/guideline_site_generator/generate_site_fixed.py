...
def generate_sidebar(root_path, output_path):
    ...
    def walk_directory(current_path, relative_path="", level=0):
        ...
        for item in sorted(current_dir.iterdir()):
            if item.suffix == '.md' and not item.name.startswith('.'):
                ...
                # 相対パスを正しく設定
                if relative_path:
                    link_path = f"./{relative_path}/{html_name}"
                else:
                    link_path = f"./{html_name}"
...
def process_file(file_path, root_path, output_path, sidebar_html, template_content):
    ...
    try:
        ...
        # 出力ファイルパスを計算（安全なファイル名を使用）
        relative_path = file_path.relative_to(root_path)
        safe_name = safe_filename(file_path.name)
        output_file = output_path / relative_path.parent / (safe_name + '.html')
...
def main():
    ...
    # サイドバーHTMLを生成
    print("📋 サイドバーを生成中...")
    sidebar_html = generate_sidebar(root_path, output_path)
